...
define(['ojs/ojcore', 'knockout','jquery', 'services/rendererService', 'services/RestService','services/exportService', 'services/MessageService', 'ojs/ojrouter',
        ...
        {
            function productEventCodeMainViewModel() {
                var self = this;
                var restService = RestService.productEventCodeService();
                self.header = "Product Event Code";
                ...
                self.search = function (code, name, desc) {
                    var tmp = self.collection().filter(function(rec){
                        return ((code.length ===0 || (code.length > 0 && rec.attributes.eventCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                (name.length ===0 || (name.length > 0 && rec.attributes.accCd.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)));
                    });
                ...
                self.activateDeactivate = function (model) {
                    ...
                    self.save(model,"Rating agency \""+model.attributes.agencyName+"\" is successfully "+(model.attributes.active==='Y'?'activated':'deactivated'));
                };
            ...
            return productEventCodeMainViewModel();
        }
); 