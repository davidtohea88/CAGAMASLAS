...
define(['ojs/ojcore', 'knockout','jquery', 'services/rendererService', 'services/RestService','services/exportService', 'services/MessageService', 'ojs/ojrouter',
        ...
        function (oj, ko, $, rendererService, RestService, exportService, MessageService)
        {
            function prchsModeMainViewModel() {
                var self = this;
                var restService = RestService.purchaseModeService();
                ...
                self.search = function (code, name, desc) {
                    var tmp = self.collection().filter(function(rec){
                        return ((code.length ===0 || (code.length > 0 && rec.attributes.prchsModeCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                (name.length ===0 || (name.length > 0 && rec.attributes.prchsModeName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                (desc.length ===0 || (desc.length > 0 && rec.attributes.prchsModeDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                    });
                ...
                self.activateDeactivate = function (model) {
                    ...
                    self.save(model,"Purchase mode \""+model.attributes.prchsModeName+"\" is successfully "+(model.attributes.active==='Y'?'activated':'deactivated'));
                };
