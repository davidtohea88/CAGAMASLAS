            ...
            function gstFeeTypeMainViewModel() {
                ...
                self.search = function (code, name, desc) {
                    var tmp = self.collection().filter(function(rec){
                        return ((code.length ===0 || (code.length > 0 && rec.attributes.gstFeetypeCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                (name.length ===0 || (name.length > 0 && rec.attributes.gstFeetypeName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                (desc.length ===0 || (desc.length > 0 && rec.attributes.gstFeetypeDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                    });
                ...
                self.save = function (model,successMsg) {
                    ...
                    model.save(defaultAttributes,{
                        success: function(model,resp){
                            self.refreshData();
                            var message = successMsg? successMsg: (model.isNew()?'A new GST Fee type is successfully created':'GST Fee type is successfully updated');
                ...
                self.activateDeactivate = function (model) {
                    ...
                    self.save(model,"GST Fee type \""+model.attributes.gstFeetypeName+"\" is successfully "+(model.attributes.active==='Y'?'activated':'deactivated'));
                };
