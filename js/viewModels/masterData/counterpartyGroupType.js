...
define(['ojs/ojcore', 'knockout', 'data/data', 'jquery', 'services/rendererService', 'services/configService','services/exportService', 'ojs/ojrouter',
        ...
        {
            function counterpartyGroupTypeMainViewModel() {
                var self = this;
                self.header = "Counterparty Group Type";
                self.dialogTitle = "Create/edit "+self.header;
                self.allData = ko.observableArray();
                self.counterPrtyGrpTypModel = ko.observable();
                self.dataSource = new oj.PagingTableDataSource(new oj.ArrayTableDataSource(self.allData, {idAttribute: 'counterPrtyGrpTypCd'}));
                ...
                self.refreshData = function (fnSuccess) {
                    ...
                    var jsonUrl = "js/data/counterpartyGroupType.json";
...
//                                headers: {"Authorization": "Basic " + btoa("username:password")},
                ...
                self.search = function (code, name, desc) {
                    var temp = ko.utils.arrayFilter(self.allData(),
                        function (rec) {
                            return ((code.length ===0 || (code.length > 0 && rec.counterPrtyGrpTypCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                    (name.length ===0 || (name.length > 0 && rec.counterPrtyGrpTypName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                    (desc.length ===0 || (desc.length > 0 && rec.counterPrtyGrpTypDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                        });
                    self.allData(temp);
                };
                
                self.createOrEdit = function (model) {
                    self.counterPrtyGrpTypModel(model);
                ...
                self.onReset = function(){
                    ...
                    self.refreshData(function(data){
                        ...
                        self.allData(data.MdCounterPrtyGrpType);
                    });
                };
                ...
                self.onCreate = function(){
                    var newRec = { counterPrtyGrpTypId: undefined,
                        counterPrtyGrpTypCd: "",
                        counterPrtyGrpTypName: "",
                        counterPrtyGrpTypDesc: "",
                        active: "Y",
                        effectiveDate: ""};
            ...
            return counterpartyGroupTypeMainViewModel();
        }
); 