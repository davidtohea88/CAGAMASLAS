...
define(['ojs/ojcore', 'knockout','jquery', 'services/rendererService', 'services/RestService','services/exportService', 'services/MessageService', 'ojs/ojrouter',
        ...
        function (oj, ko, $, rendererService, RestService, exportService, MessageService)
        {
            function vendorMainViewModel() {
                ...
                districtService.fetchAsLOV('distName','distId').then(function(data){
                    self.districtLOV(data);
                });
                self.selectedDistrictId = ko.observableArray();
                ...
                self.search = function (code, name, desc) {
                    var tmp = self.collection().filter(function(rec){
                        return ((code.length ===0 || (code.length > 0 && rec.attributes.vndrCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                (name.length ===0 || (name.length > 0 && rec.attributes.vndrName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                (desc.length ===0 || (desc.length > 0 && rec.attributes.addrDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                    });
                ...
                self.activateDeactivate = function (model) {
                    ...
                    self.save(model,self.header+" \""+model.attributes.vndrCd+"\" is successfully "+(model.attributes.active==='Y'?'activated':'deactivated'));
                };
                ...
                self.onEdit = function(){
                    ...
                    self.selectedDistrictId([model.attributes.distId]);
                    self.createOrEdit(model);
                };
                ...
                self.onSave = function(){
                    ...
                    if (!(trackerObj.invalidHidden || trackerObj.invalidShown)){
                        self.model().attributes.distId = self.selectedDistrictId()[0];
