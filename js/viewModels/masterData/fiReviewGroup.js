...
define(['ojs/ojcore', 'knockout','jquery', 'services/rendererService', 'services/RestService','services/exportService', 'services/MessageService', 'ojs/ojrouter',
        ...
        {
            function fiReviewGroupMainViewModel() {
                var self = this;
                var restService = RestService.fiReviewGroupService();
                self.header = "FI Review Group";
                ...
                self.search = function (code, name, desc) {
                    var tmp = self.collection().filter(function(rec){
                        return ((code.length ===0 || (code.length > 0 && rec.attributes.rvwGrpCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                (name.length ===0 || (name.length > 0 && rec.attributes.rvwGrpName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                (desc.length ===0 || (desc.length > 0 && rec.attributes.rvwGrpDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                    });
                ...
                self.activateDeactivate = function (model) {
                    ...
                    self.save(model,self.header+" \""+model.attributes.rvwGrpName+"\" is successfully "+(model.attributes.active==='Y'?'activated':'deactivated'));
                };
            ...
            return fiReviewGroupMainViewModel();
        }
); 