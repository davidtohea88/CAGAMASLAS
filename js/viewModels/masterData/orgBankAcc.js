...
define(['ojs/ojcore', 'knockout','jquery', 'services/rendererService', 'services/RestService','services/exportService', 'services/MessageService', 'ojs/ojrouter',
        ...
        {
            function orgBankAccMainViewModel() {
                var self = this;
                // LOV
                var organizationService = RestService.organizationService();
                self.organizationLOV = ko.observableArray();
                organizationService.fetchAsLOV('orgName','orgId').then(function(data){
                    self.organizationLOV(data);
                });
                self.selectedOrganizationId = ko.observableArray();
                ...
                // Service
                var restService = RestService.orgBankAccService();
                self.header = "Organization Bank Account";
                ...
                self.accNumberSearch = ko.observable('');
                ...
                self.search = function (name, accNumber) {
                    var tmp = self.collection().filter(function(rec){
                        return ((name.length ===0 || (name.length > 0 && rec.attributes.bankName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                (accNumber.length ===0 || (accNumber.length > 0 && rec.attributes.bankAccNo.toLowerCase().indexOf(accNumber.toString().toLowerCase()) > -1)));
                    });
                ...
                self.activateDeactivate = function (model) {
                    ...
                    self.save(model,self.header+" \""+model.attributes.bankName+"\" is successfully "+(model.attributes.active==='Y'?'activated':'deactivated'));
                };
                ...
                self.onSearch = function(){
                    self.collection().fetch({
                        success: function(){
                            self.search(self.nameSearch(),self.accNumberSearch());
                        },error: function(resp){
                            self.showMessage("ERROR",MessageService.httpStatusToMessage(resp.status));
                        }
                    });
                ...
                self.onEdit = function(){
                    ...
                    self.selectedOrganizationId([model.attributes.orgId]);
                    self.createOrEdit(model);
                };
                ...
                self.onSave = function(){
                    ...
                    if (!(trackerObj.invalidHidden || trackerObj.invalidShown)){
                        ...
                        self.model().attributes.prodTypeId = self.selectedOrganizationId()[0];
            ...
            return orgBankAccMainViewModel();
        }
); 