...
define(['ojs/ojcore', 'knockout', 'jquery', 'services/rendererService', 'services/RestService','services/exportService', 'ojs/ojrouter',
        ...
        function (oj, ko, $, rendererService, RestService, exportService)
        {
            function gstTypeMainViewModel() {
                ...
                self.gstTypeModel = ko.observable();
                self.dataSource = new oj.PagingTableDataSource(new oj.ArrayTableDataSource(self.allData, {idAttribute: 'gstTypCd'}));
                ...
                self.refreshData = function (fnSuccess) {
                    ...
                    var jsonUrl = "js/data/gstType.json";
...
//                                headers: {"Authorization": "Basic " + btoa("username:password")},
                ...
                self.search = function (code, name, desc) {
                    var temp = ko.utils.arrayFilter(self.allData(),
                        function (rec) {
                            return ((code.length ===0 || (code.length > 0 && rec.gstTypCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                    (name.length ===0 || (name.length > 0 && rec.gstTypName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                    (desc.length ===0 || (desc.length > 0 && rec.gstTypDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                        });
                    self.allData(temp);
                };
                
                self.createOrEdit = function (model) {
                    self.gstTypeModel(model);
