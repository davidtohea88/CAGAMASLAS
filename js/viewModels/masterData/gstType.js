...
define(['ojs/ojcore', 'knockout', 'jquery', 'services/rendererService', 'services/RestService', 'services/exportService','services/MessageService', 'ojs/ojrouter',
        ...
        function (oj, ko, $, rendererService, RestService, exportService,MessageService)
        {
            function gstTypeMainViewModel() {
                var self = this;
                var restService = RestService.gstTypeService();
                ...
                self.search = function (code, name, desc) {
                    var tmp = self.collection().filter(function(rec){
                        return ((code.length ===0 || (code.length > 0 && rec.attributes.gstTypeCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                (name.length ===0 || (name.length > 0 && rec.attributes.gstTypeName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                (desc.length ===0 || (desc.length > 0 && rec.attributes.gstTypeDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                    });
                ...
                self.save = function (model, successMsg) {
                    ...
                    model.save(defaultAttributes,{
                        success: function(model,resp){
                            self.refreshData();
                            var message = successMsg? successMsg: (model.isNew()?'A new GST Type is successfully created':'GST Type is successfully updated');
                ...
                self.activateDeactivate = function (model) {
                    ...
                    self.save(model,"GST Type \""+model.attributes.gstTypeName+"\" is successfully "+(model.attributes.active==='Y'?'activated':'deactivated'));
                };
