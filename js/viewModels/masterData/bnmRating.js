...
define(['ojs/ojcore', 'knockout','jquery', 'services/rendererService', 'services/RestService','services/exportService', 'services/MessageService', 'ojs/ojrouter',
        ...
        {
            function bnmRatingMainViewModel() {
                var self = this;
                // LOV
                ...
                // Service
                var restService = RestService.bnmRatingService();
                self.header = "BNM Rating";
                ...
                self.search = function (code, name, desc) {
                    var tmp = self.collection().filter(function(rec){
                        return ((code.length ===0 || (code.length > 0 && rec.attributes.bnmRateCatgy.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                (name.length ===0 || (name.length > 0 && rec.attributes.bnmRateWeight.toString().toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                (desc.length ===0 || (desc.length > 0 && rec.attributes.agencyBnmRating.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                    });
                ...
                self.activateDeactivate = function (model) {
                    ...
                    self.save(model,self.header+" \""+model.attributes.bnmRatingCd+"\" is successfully "+(model.attributes.active==='Y'?'activated':'deactivated'));
                };
                ...
                self.onSave = function(){
                    ...
                    if (!(trackerObj.invalidHidden || trackerObj.invalidShown)){
                        self.model().attributes.cptTypeId = self.selectedCounterpartyTypeId()[0];
