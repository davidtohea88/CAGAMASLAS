...
define(['ojs/ojcore', 'knockout', 'data/data', 'jquery', 'services/rendererService', 'services/configService','services/exportService', 'ojs/ojrouter',
        ...
        function (oj, ko, data, $, rendererService, configService, exportService)
        {
            function prchsConsTypMainViewModel() {
                ...
                self.dataSource = new oj.PagingTableDataSource(new oj.ArrayTableDataSource(self.allData, {idAttribute: 'prchsConsTypCd'}));
                ...
                self.refreshData = function (fnSuccess) {
                    ...
                    var jsonUrl = "js/data/purchaseConsTyp.json";
...
//                                headers: {"Authorization": "Basic " + btoa("username:password")},
                ...
                self.search = function (code, name, desc) {
                    var temp = ko.utils.arrayFilter(self.allData(),
                        function (rec) {
                            return ((code.length ===0 || (code.length > 0 && rec.prchsConsTypCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                    (name.length ===0 || (name.length > 0 && rec.prchsConsTypName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                    (desc.length ===0 || (desc.length > 0 && rec.prchsConsTypDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                        });
                    self.allData(temp);
                };
                ...
                self.onReset = function(){
                    ...
                    self.refreshData(function(data){
                        ...
                        self.allData(data.MdPrchsConsTyp);
                    });
                };
                ...
                self.onCreate = function(){
                    var newRec = { prchsConsTypId: undefined,
                        prchsConsTypCd: "",
                        prchsConsTypName: "",
                        prchsConsTypDesc: "",
                        active: "Y",
