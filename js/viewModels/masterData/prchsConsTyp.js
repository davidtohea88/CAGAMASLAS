...
define(['ojs/ojcore', 'knockout', 'jquery', 'services/rendererService', 'services/RestService','services/exportService', 'ojs/ojrouter',
        ...
        function (oj, ko, $, rendererService, RestService, exportService)
        {
            function prchsConsTypMainViewModel() {
                var self = this;
                //LOV
                var prodGroupService = RestService.productGroupService();
                self.productGroupLOV = ko.observableArray();
                prodGroupService.fetchAsLOV('prodGrpName','prodGrpId').then(function(data){
                    self.productGroupLOV(data);
                });
                self.selectedProductGroupId = ko.observableArray();
                
                var restService = RestService.purchaseConsTypeService();
                ...
                self.prchsConsTypModel = ko.observable();
                ...
                self.search = function (code, name, desc) {
                    var tmp = self.collection().filter(function(rec){
                        return ((code.length ===0 || (code.length > 0 && rec.attributes.consTypeCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                (name.length ===0 || (name.length > 0 && rec.attributes.consTypeName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                (desc.length ===0 || (desc.length > 0 && rec.attributes.consTypeDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                    });
                ...
                self.onEdit = function(){
                    ...
                    console.log(self.productGroupLOV().indexOf(model.attributes.prodGrpId));
                    self.selectedProductGroupId([model.attributes.prodGrpId]);
                    self.createOrEdit(model);
                };
                
                self.onSave = function(){
                    self.prchsConsTypModel().attributes.prodGrpId = self.selectedProductGroupId()[0];
                    self.save(self.prchsConsTypModel());
