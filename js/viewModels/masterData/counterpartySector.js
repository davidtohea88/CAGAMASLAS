...
define(['ojs/ojcore', 'knockout', 'data/data', 'jquery', 'services/rendererService', 'services/configService','services/exportService', 'ojs/ojrouter',
        ...
        {
            function counterpartySectorMainViewModel() {
                var self = this;
                self.header = "Counterparty Sector";
                self.dialogTitle = "Create/edit "+self.header;
                self.allData = ko.observableArray();
                self.counterpartySectorModel = ko.observable();
                self.dataSource = new oj.PagingTableDataSource(new oj.ArrayTableDataSource(self.allData, {idAttribute: 'counterPrtySectCd'}));
                ...
                self.refreshData = function (fnSuccess) {
                    ...
                    var jsonUrl = "js/data/counterpartySector.json";
...
//                                headers: {"Authorization": "Basic " + btoa("username:password")},
                ...
                self.search = function (code, name, desc) {
                    var temp = ko.utils.arrayFilter(self.allData(),
                        function (rec) {
                            return ((code.length ===0 || (code.length > 0 && rec.counterPrtySectCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                    (name.length ===0 || (name.length > 0 && rec.counterPrtySectName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                    (desc.length ===0 || (desc.length > 0 && rec.counterPrtySectDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                        });
                    self.allData(temp);
                };
                
                self.createOrEdit = function (model) {
                    self.counterpartySectorModel(model);
                ...
                self.onReset = function(){
                    ...
                    self.refreshData(function(data){
                        ...
                        self.allData(data.MdCounterPrtySector);
                    });
                };
                ...
                self.onCreate = function(){
                    var newRec = { counterPrtySectId: undefined,
                        counterPrtySectCd: "",
                        counterPrtySectName: "",
                        counterPrtySectDesc: "",
                        active: "Y",
            ...
            return counterpartySectorMainViewModel();
        }
); 