...
define(['ojs/ojcore', 'knockout', 'data/data', 'jquery', 'services/rendererService', 'services/configService','services/exportService', 'ojs/ojrouter',
        ...
        {
            function counterpartyGroupMainViewModel() {
                var self = this;
                self.header = "Counterparty Group";
                self.dialogTitle = "Create/edit "+self.header;
                self.allData = ko.observableArray();
                self.counterpartyGroupModel = ko.observable();
                self.dataSource = new oj.PagingTableDataSource(new oj.ArrayTableDataSource(self.allData, {idAttribute: 'counterPrtyGrpCd'}));
                ...
                self.refreshData = function (fnSuccess) {
                    ...
                    var jsonUrl = "js/data/counterpartyGroup.json";
...
//                                headers: {"Authorization": "Basic " + btoa("username:password")},
                ...
                self.search = function (code, name, desc) {
                    var temp = ko.utils.arrayFilter(self.allData(),
                        function (rec) {
                            return ((code.length ===0 || (code.length > 0 && rec.counterPrtyGrpCd.toLowerCase().indexOf(code.toString().toLowerCase()) > -1)) &&
                                    (name.length ===0 || (name.length > 0 && rec.counterPrtyGrpName.toLowerCase().indexOf(name.toString().toLowerCase()) > -1)) &&
                                    (desc.length ===0 || (desc.length > 0 && rec.counterPrtyGrpDesc.toLowerCase().indexOf(desc.toString().toLowerCase()) > -1)));
                        });
                    self.allData(temp);
                };
                
                self.createOrEdit = function (model) {
                    self.counterpartyGroupModel(model);
                ...
                self.onReset = function(){
                    ...
                    self.refreshData(function(data){
                        ...
                        self.allData(data.MdCounterPrtyGrp);
                    });
                };
                ...
                self.onCreate = function(){
                    var newRec = { counterPrtyGrpId: undefined,
                        counterPrtyGrpCd: "",
                        counterPrtyGrpName: "",
                        counterPrtyGrpDesc: "",
                        active: "Y",
            ...
            return counterpartyGroupMainViewModel();
        }
); 